...
getting_situated1 <- function(envir) {
  ...
  if (is.null(integration)) {
    stop("The argument 'integration' must be specified.")
  } # TOFIX check that it is from the right set of functions!
}
...
getting_situated2 <- function(envir) {
  ...
  if (separate_interactions && !model[["model_specification"]][["regs"]][["interactions"]][["present"]]) {
...
empirical_Int_catmet_handling <- function(envir) {
  ...
  if (continue_categorical) {
    assign("RItype", "categorical", envir = envir)
  } else {
    assign("RItype", "metric", envir = envir)
  }
}
